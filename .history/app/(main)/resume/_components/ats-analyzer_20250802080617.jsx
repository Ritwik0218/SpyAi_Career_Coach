...
export function ATSAnalyzer({ currentResume, onResumeUpdate }) {
  ...
  const handleFileUpload = async (event) => {
    ...
    const { actionId } = startAction('file-extraction', {
    ...
    try {
      ...
      updateProgress(actionId, 25, "Reading file...");
      
      const extractedText = await extractTextFromFile(file);
      
      updateProgress(actionId, 75, "Processing content...");
      ...
      updateProgress(actionId, 100, "Extraction complete!");
      ...
      toast.error(
        error.message || "Failed to extract text from file. Please try manual input."
      );
    }
  };
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                📄 Upload .txt, .pdf, or .docx file (max 10MB) - completely FREE!
              </p>
            </div>
            ...
            {isExtracting && (
              <div className="space-y-3 p-4 bg-card border rounded-lg">
                <MiniLoader 
                  ...
                  progress={getActionState('file-extraction')?.progress || 0}
                  variant="progress"
                  size="md"
                >
                  {getActionState('file-extraction')?.description || "Processing..."}
                </MiniLoader>
              </div>
            )}
            
            {uploadedResume && !isExtracting && (
              <div className="p-3 bg-muted rounded-lg">
                <MiniLoader success size="sm">
                  ✅ File extracted successfully! ({uploadedResume.length} characters)
                </MiniLoader>
              </div>
            )}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Or paste your resume content directly:</label>
              <Textarea
              ...
              />
              <p className="text-xs text-muted-foreground">
                💡 If PDF upload doesn't work, you can copy and paste your resume text here
              </p>
            </div>
          </CardContent>
        </Card>
