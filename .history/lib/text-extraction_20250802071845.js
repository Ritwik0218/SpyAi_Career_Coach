...
export async function extractTextFromFile(file) {
  ...
  // Handle PDF files with client-side processing
  if (
    fileType === 'application/pdf' || 
    fileName.endsWith('.pdf')
  ) {
    try {
      // Use client-side PDF processing to avoid server-side issues
      const { extractPDFTextClient } = await import('@/lib/client-pdf-utils');
      return await extractPDFTextClient(file);
    } catch (error) {
      console.error('PDF extraction error:', error);
      throw new Error('Failed to extract text from PDF. The PDF might be image-based or protected. Please convert to DOCX or copy the text manually.');
    }
  }
